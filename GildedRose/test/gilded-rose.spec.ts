...
describe('Gilded Rose', function () {
    ...
    it('`Aged Brie` should decrease the SellIn by 1 and increase the value by 1', function() {
        const gildedRose = new GildedRose([ new Item(TypeItem.AGED_BRIE, 1, 0), new Item(TypeItem.AGED_BRIE, 0, 0), new Item(TypeItem.AGED_BRIE, 10, 0),
            new Item(TypeItem.AGED_BRIE, 10, 49), new Item(TypeItem.AGED_BRIE, 10, 50) ]);
        const items = gildedRose.updateQuality();
    ...
    it('`Sulfuras` should not be modified with the updates', function() {
        const gildedRose = new GildedRose([ new Item(TypeItem.SULFURAS, 1, 0), new Item(TypeItem.SULFURAS, 0, 0), new Item(TypeItem.SULFURAS, 10, 0)]);
        const items = gildedRose.updateQuality();
    ...
    it('`Backstage passes` Quality increases by 2 when there are 10 days or less and by 3 when there are 5 days or less but Quality drops to 0 after the concert', function() {
        const gildedRose = new GildedRose([ new Item(TypeItem.BACKSTAGE_PASS, 1, 0), new Item(TypeItem.BACKSTAGE_PASS, 0, 0), new Item(TypeItem.BACKSTAGE_PASS, 10, 0),
            new Item(TypeItem.BACKSTAGE_PASS, 15, 20), new Item(TypeItem.BACKSTAGE_PASS, 10, 49), new Item(TypeItem.BACKSTAGE_PASS, 5, 49)]);
        const items = gildedRose.updateQuality();
